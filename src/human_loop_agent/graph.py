...
def call_llm(state: ChatbotState, config: RunnableConfig) -> Command:
    if not state.messages:
        state.messages = [{"role": "user", "content": "Hello! Let's start."}]

    configuration = Configuration.from_runnable_config(config)
    model = load_chat_model(configuration.model)

    system_message = {"role": "system", "content": SYSTEM_PROMPT}
    messages_with_prompt = [system_message] + state.messages

    response = model.invoke(messages_with_prompt)

    state.messages = response
    return Command(goto="generate_question")


def generate_question(state: ChatbotState) -> Command:
    if not state.messages:
        state.messages = [
            {"role": "user", "content": "No prior conversation detected."}
        ]
    ...
    state.messages = interrupt({"question": questions}) or state.messages
